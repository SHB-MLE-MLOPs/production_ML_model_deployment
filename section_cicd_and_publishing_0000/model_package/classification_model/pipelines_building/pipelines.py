...
import sys
sys.path.append(
    str(Path().absolute().parent.parent))  # this command beginning in the folder of file pipelines.py everywhere you launch it, this path is in the folder section-production-model-package
...
titanic_pipe_DP = Pipeline([
    ...
    ('replace_mv_by_NAN', h_pp.ReplaceMissingValueByNanTransform(
        missing_values_list=core.config.mod_config.search_missing_value_list)),

    # == CASTED SOME NUMERICAL VARIABLE AS FLOAT  ====
    ('some_nv_casted', h_pp.CastingNumericalVariableAsFloat(
    ...
    ('retain_first', h_pp.RetainingFirstOfCabinTransform(
    ...
    ('extract_title', h_pp.ExtractionTitleFromTheNameTransform(
        variable_list=core.config.mod_config.categorical_variables_extract_title)),

    # == DROP UNNECESSARY VARIABLES FROM DATA SET  ====
    ('drop_features', DropFeatures(features_to_drop=core.config.mod_config.drop_variables)),
])


titanic_pipe_AE_bis = Pipeline(
    [
        # == SEARCHING MISSING VALUE IN DATA SET AND REPLACE THEM BY NAN  ====
        (
            "replace_mv_by_NAN",
        ...
        (
            "drop_features",
            DropFeatures(
                features_to_drop=core.config.mod_config.drop_variables,
            ),
        ),
    ]
)
...
titanic_pipe_FE = Pipeline([
    ...
    ('missing_indicator', AddMissingIndicator(variables=core.config.mod_config.numerical_variables_with_na)),

    # impute missing value in numerical variables with the median
    ('mean_imputation', MeanMedianImputer(
        imputation_method='median', variables=core.config.mod_config.numerical_variables_with_na)),

    # impute missing value in categorical variables (witch have higher percentage of missing values) with string
    # 'missing'
    ('missing_imputation', CategoricalImputer(
        imputation_method='missing', variables=core.config.mod_config.categorical_variables_with_na_missing)),

    # impute missing value in categorical variables (witch have few percentage of missing values) with frequent value
    ('frequent_imputation', CategoricalImputer(
        imputation_method='frequent', variables=core.config.mod_config.categorical_variables_with_na_frequent)),

    # ==== OUTLIERS NUMERICAL VARIABLE TRANSFORMATION =====
    # create a new feature for presence or absence of outliers
    ('outliers_feature_creation', h_pp.OutliersFeatureCreation(outliers_num_vars_list=core.config.mod_config.outliers_variables)),

    # ==== NUMERICAL VARIABLE TRANSFORMATION =====
    # for variable witch becomes unskewed after apply transformation method
    ('quantile', SklearnTransformerWrapper(
        transformer=QuantileTransformer(random_state=core.config.mod_config.random_state), variables=core.config.mod_config.numerical_quantile_variables)),

    # for variable that remains skewed even apply transformation method
    ('binarizer', SklearnTransformerWrapper(
        transformer=Binarizer(threshold=0), variables=core.config.mod_config.binarize_variables)),

    # == CATEGORICAL ENCODING ======
    # remove categories present in less than 5% of the observations (0.05)
    # group them in one category called 'Rare'
    ('rare_label_encoder', RareLabelEncoder(
        tol=0.05, n_categories=1, variables=core.config.mod_config.rare_variables)),
...
estimator = LogisticRegression(C=core.config.mod_config.C, random_state=core.config.mod_config.random_state)
