...
sys.path.append(
    str(Path().absolute().parent.parent))  # this command beginning in the folder of file predict.py everywhere you launch it, this path is in the folder section-production-model-package
...
# Here, input_data has DataFrame or dict type.
# So we can give an input_data witch can contain or not missing value like "?" for example as input of function make_prediction.
# In the case input_data has type like new_predict_data_preparation.List[TitanicDataInputSchema],
# it required that you give an input_data witch respect type of each element in TitanicDataInputSchema
def make_prediction(*, input_data: t.Union[pd.DataFrame, dict]) -> dict:
    ...
    if not errors:
        ...
        # ========== USING OF DATA PREPARATION PIPELINE ==========
        ...
        # ========== FEATURES DATA WITCH ARE READY FOR FEATURE ENGINEERING STEP ==========
        # validated_data_features_dp = validated_data[core.config.mod_config.initial_features]
