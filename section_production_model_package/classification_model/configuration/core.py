...
import sys
sys.path.append(
    str(Path().absolute().parent.parent))  # this command beginning in the folder of file core.py everywhere you launch it, this path is in the folder section-production-model-package
import classification_model
# import yaml
# import yamlordereddictloader
# from ... import classification_model  # to import from 2 directory above the folder than the script is running
# import for_path


# Project Directories
# root of "classification_model" folder
# PACKAGE_ROOT = Path().absolute().parent.parent # this command beginning in the folder when you launch it
PACKAGE_ROOT = Path(classification_model.__file__).absolute().parent.parent # this command beginning in the folder of file core.py everywhere you launch it, this path is in the folder section-production-model-package
...
# CONFIG_FILE_PATH = Path(str(PACKAGE_ROOT / "classification_model" / "config.yml"))
TRAINED_MODEL_DIR = PACKAGE_ROOT / "classification_model" / "save_trained_model_elements"
...
# Configuration will be used like application in the model, like pipeline, train and test data, ...
...
def create_and_validate_config(parsed_config: YAML = None) -> MasterConfig:
    """check the type of variable witch will be pass in the function"""
    if not isinstance(parsed_config, Union[YAML, None]):
